...
public class HigherOrderFunctionsLearningTest {
    ...
    public void testMap() {
        ...
        List<Integer> actual = input.map(number -> number+1).collect(Collectors.toList()); // TODO: Replace the `actual` with your solution, here and in all the following.
    ...
    public void testDistinct() {
        Stream<String> input = Stream.of("a1", "a2", "a3", "b1", "b2", "b3");
        List<String> actual = input.map(x -> String.valueOf((x.charAt(0)))).distinct().collect(Collectors.toList());
    ...
    public void testConcatEven() {
        Stream<String> input = Stream.of("a1", "a2", "a3", "b1", "b2", "b3");
        String actual = input.filter(string-> Character.getNumericValue(string.charAt(1))%2 ==0).collect(Collectors.joining());
    ...
    public void testDetermineMostActiveMonth() {
        Stream<Revision> input = getRevisions("soup30.json");
        Month actual = Collections.max(input.collect(Collectors.groupingBy(revision ->revision.timestamp.atZone(ZoneId.of("UTC")).getMonth(),
